...
int ParseConfigFile(FILE *f_ini, WaveDumpConfig_t *WDcfg) 
{
	...
	while(!feof(f_ini)) {
        ...
        // Mysql server
		if (strstr(str, "MYSQL_SERVER")!=NULL) {
			read = fscanf(f_ini, "%s", WDcfg->MysqlServer);
			continue;
		}
        // Mysql user
		if (strstr(str, "MYSQL_USER")!=NULL) {
			read = fscanf(f_ini, "%s", WDcfg->MysqlUser);
			continue;
		}
        // Mysql password
		if (strstr(str, "MYSQL_PASSWORD")!=NULL) {
			read = fscanf(f_ini, "%s", WDcfg->MysqlPassword);
			continue;
		}
        // Mysql database
		if (strstr(str, "MYSQL_DATABASE")!=NULL) {
			read = fscanf(f_ini, "%s", WDcfg->MysqlDatabase);
			continue;
		}
	//Output File Folder
		time_t     now;
		...
		time(&now);
		ts = *localtime(&now);
		strftime(buf, sizeof(buf), "%Y_%m_%d_%H_%M", &ts);
		if (strstr(str, "OUTPUT_PATH")!=NULL) {
			read = fscanf(f_ini, "%s", WDcfg->OutputPath);
			//create folder if not exists
			char path[256];
			sprintf(path, "%s/%s", WDcfg->OutputPath, buf);
			mkdir(path, 0775);
			//update OutputPath to point to Kasia's datetime folder
			strncpy(WDcfg->OutputPath, path, 256);
			continue;
		}
	//Acquisition Time
		if (strstr(str, "ACQUISITION_TIME")!=NULL) {
			read = fscanf(f_ini, "%d", &WDcfg->AcquisitionTime);
			continue;
		}
	//Readout Period
		if (strstr(str, "READOUT_PERIOD")!=NULL) {
			read = fscanf(f_ini, "%d", &WDcfg->ReadoutPeriod);
			continue;
		}
	...
	//Choose channels within group to readout
		if (strstr(str, "READOUT_CHANNEL")!=NULL) ///xx742
		{
			uint8_t ok[8];
			read = fscanf(f_ini, "%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd,%hhd", &ok[0], &ok[1], &ok[2], &ok[3], &ok[4], &ok[5], &ok[6], &ok[7]);
			for(uint8_t i=0; i<8; ++i) {
				WDcfg->ReadoutChannel[ch][i] = ok[i];
			}
			continue;
		}

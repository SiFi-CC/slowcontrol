...
#define WaveDump_Release        "3.10.2"
#define WaveDump_Release_Date   "February 2021"
#define DBG_TIME
...
static int ext_idx = -1;
static char ext_command[10] = { 0 };
...
//        if (ext_idx > 8) return;
//        ext_command[ext_idx+1] = 'W';
//        ext_command[ext_idx+2] = 's';
//        ext_idx += 2;
//    }
//    if (signum == SIGUSR2)
//    {
...
//        if (ext_idx > 7) return;
//        ext_command[ext_idx+1] = 'q';
//        ext_command[ext_idx+2] = 's';
//        ext_command[ext_idx+3] = 'W';
//        ext_idx += 3;
//    }
//}
...
int WriteOutputFilesx742(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, CAEN_DGTZ_X742_EVENT_t *Event)
{
    ...
    for (gr=0;gr<(WDcfg->Nch/8);gr++) {
        if (Event->GrPresent[gr]) {
            for(ch=0; ch<9; ch++) {
		if(!WDcfg->ReadoutChannel[gr][ch]) continue;
			...
			char path[100];
			sprintf(path, "%s/%s", WDcfg->OutputPath, fname);
                        if ((WDrun->fout[(gr*9+ch)] = fopen(path, "wb")) == NULL)
                            return -1;
                    }
                    if( WDcfg->OutFileFlags & OFF_HEADER) {
                        // Write the Channel Header
...
{
	int total_acquisition_time = 60; //seconds
	char current_runid[8];
//    signal(SIGUSR1, my_handler);
//    signal(SIGUSR2, m5y_handler);
    ...
    uint64_t CurrentTime, PrevRateTime, ElapsedTime, StartAcquisitionTime;
    int nCycles= 0;
    ...
    MYSQL_RES *result;
    if (WDcfg.UseMysql) {
	    const char *server = WDcfg.MysqlServer;
	    const char *user = WDcfg.MysqlUser;
	    const char *password = WDcfg.MysqlPassword;
	    const char *database = WDcfg.MysqlDatabase;
	    if (!mysql_real_connect(conn, server,
				    user, password, database, 0, NULL, 0)) {
		    fprintf(stderr, "%s\n", mysql_error(conn));
		    exit(1);
	    }
	    printf("Connected to the local MySQL server\n");
	    mysql_query(conn, "INSERT INTO runs (detector_id) VALUES (1);"); 
	    printf("Creating new run...\n");
	    if (mysql_query(conn, "SELECT LPAD(LAST_INSERT_ID(), 5, 0);") ) {
		    printf("%s\n", mysql_error(conn) );
	    } else {
		    ...
		    if (result)  // there are rows
		    {
			    ...
			    strncpy(current_runid, mysql_fetch_row(result)[0], sizeof(mysql_fetch_row(result)[0]) );
			    mysql_free_result(result);
		    }
	    }
    }
    char f_runid[100];
    sprintf(f_runid, "%s/%s", WDcfg.OutputPath, current_runid);
    FILE *fd = fopen(f_runid, "w+");
    fputs("", fd);
    fclose(fd);
...
InterruptTimeout:
        ...
        Nb += BufferSize;
        Ne += NumEvents;
        CurrentTime = get_time(); //in ms
        ElapsedTime = CurrentTime - PrevRateTime; //in ms

        nCycles++;
        if (ElapsedTime > WDcfg.ReadoutPeriod) { //changed from 1000 ms
		if((CurrentTime-StartAcquisitionTime)/1000 > WDcfg.AcquisitionTime) {
    		    printf("autostop!\n");
    		    if (ext_idx > 7) return;
    		    ext_command[ext_idx+1] = 'q';
    		    ext_command[ext_idx+2] = 's';
    		    ...
    		    ext_idx += 3;
                    ...
                    mysql_query(conn, "UPDATE runs SET state='test' WHERE id IN (SELECT * FROM(SELECT id FROM runs ORDER BY id DESC LIMIT 1)tblTmp);"); 
		    printf("stopping run\n");
    		}
            if (Nb == 0) {
		    ...
		    printf("No data...\n");
		    if (WDcfg.UseMysql) {
			if (mysql_query(conn, "INSERT INTO outputs (device_id, value) VALUES ((SELECT id FROM devices WHERE name='CAENDigitizer' AND unit='Hz'), 0);") ) {
			        printf("%s\n", mysql_error(conn) );
			        break;
			}
			if (mysql_query(conn, "INSERT INTO outputs (device_id, value) VALUES ((SELECT id FROM devices WHERE name='CAENDigitizer' AND unit='MB'), 0);" ) ) {
			        printf("%s\n", mysql_error(conn) );
			        break;
			}
		    }
		...
		float freq = (float)(Ne*WDcfg.ReadoutPeriod)/(float)ElapsedTime;
		float size = (float)Nb/((float)(ElapsedTime*WDcfg.ReadoutPeriod*0.48576f) );
                printf("Reading at %.2f MB/s (Trg Rate: %.2f Hz)\n", size, freq);
		if (WDcfg.UseMysql) {
		    char query[200];
                    sprintf(query, "INSERT INTO outputs (device_id, value) VALUES ((SELECT id FROM devices WHERE name='CAENDigitizer' AND unit='Hz'), %f);", freq);
                    if (mysql_query(conn, query ) ) {
                        printf("%s\n", mysql_error(conn) );
                        break;
                    }
                    sprintf(query, "INSERT INTO outputs (device_id, value) VALUES ((SELECT id FROM devices WHERE name='CAENDigitizer' AND unit='MB'), %f);", size);
                    if (mysql_query(conn, query ) ) {
                        printf("%s\n", mysql_error(conn) );
                        break;
                    }
